...
class MultiProcessor(ABC):
    ...
    def run(self, params, num_devices, sequential_order=False, node_index=0, num_nodes=1):
        """
        ...
        tasks = tasks[node_index::num_nodes]
        print(f"Node {node_index}/{num_nodes}: Processing {len(tasks)} tasks {tasks[:2]=}")
